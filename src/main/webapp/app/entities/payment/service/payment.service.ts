...
type RestOf<T extends IPayment | NewPayment> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class PaymentService {
  ...
  protected convertDateFromClient<T extends IPayment | NewPayment | PartialUpdatePayment>(payment: T): RestOf<T> {
    return {
      ...payment,
      createdDate: payment.createdDate?.toJSON() ?? null,
      lastModifiedDate: payment.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...restPayment,
      createdDate: restPayment.createdDate ? dayjs(restPayment.createdDate) : undefined,
      lastModifiedDate: restPayment.lastModifiedDate ? dayjs(restPayment.lastModifiedDate) : undefined,
    };
  }
